...
def get_cvae_filename(cmax, losstype, file_list):
    for fn in file_list:
        ...
        if("Cmax" + str(cmax) in fn and losstype in fn and 'bern' in fn):
            return fn
    return None
...
loss_filenames = [f for f in os.listdir(loss_file_path) if f[-4:] =='.npy' and not "alldata" in f and not "nois" in f]
