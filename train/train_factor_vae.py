...
def train_factor_VAE(model, discriminator, vae_optimizer, discriminator_optimizer, gamma, nb_epoch, train_loader, viz = False):
    ...
    for epoch in range(nb_epoch):
        ...
        for data1, data2 in train_loader:
            ...
            train_recon_loss+= recon_l* data1.size(0)

            kld_l = kld_loss(mu1, logvar1)
            train_kld_loss+= kld_l* data1.size(0)

            dz = discriminator(z1)
            tc_l = tc_loss(dz)
            train_tc_loss+= tc_l* data1.size(0)

            total_loss = recon_l + kld_l + torch.mul(gamma, tc_l)
            train_vae_loss += total_loss * data1.size(0)
            ...
            train_d_loss += discriminator_l * data1.size(0)
    ...
    if viz:
        plt.plot(np.arange(1,nb_epoch+1,1), train_vae_losses)
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
